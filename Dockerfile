...
RUN openssl req -newkey rsa:4096 -x509 -sha256 -days 365 -nodes -out /etc/nginx/ssl/phpMyAdmin.pem -keyout /etc/nginx/ssl/phpMyAdmin.key \
    -subj "/C=BE/ST=Bruxelles/L=bruxelles/O=19/CN=vbeaufay"
...
RUN openssl req -newkey rsa:4096 -x509 -sha256 -days 365 -nodes -out /etc/nginx/ssl/wordpress.pem -keyout /etc/nginx/ssl/wordpress.key \
    -subj "/C=BE/ST=Bruxelles/L=bruxelles/O=19/CN=vbeaufay"
